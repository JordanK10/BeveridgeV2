...
def _require_len(series, name):
    n = len(series) if hasattr(series, "__len__") else len(np.asarray(series))
    if n != len(TIME):
        raise ValueError(f"{name}: length {n} != len(TIME) {len(TIME)} (calendar-aligned GDP required)")
...
if "gdp" in RAW_GDP.columns:
    ...
    _require_len(gdp_interp, "gdp_interp")
...
try:
    ...
    _require_len(ar2_interp, "ar2_interp")
