...
def plot_gdp_shock_response_figure(
    ...
    for j, g in enumerate(g_values):
        color = cmap(norm(g))
        axs[0, 0].plot(np.log(tau_axis), np.log(U_by_g[j]), color=color, label=f"g={g:.2f}", linewidth=1.2, alpha=0.9)
        axs[1, 0].plot(np.log(tau_axis), np.log(V_by_g[j]), color=color, label=f"g={g:.2f}", linewidth=1.2, alpha=0.9)
    ...
    axs[0, 1].scatter(g_arr[valid_U], np.asarray(beta_U)[valid_U], c="C0", s=36, zorder=3, label=r"$\beta_U$")
    axs[0, 1].set_title(r"Power-law exponent $\beta_U$ vs shock level $g$")
    axs[0, 1].set_xlabel(r"Post-shock signal $g$")
    axs[0, 1].set_ylabel(r"$\beta_U$ (log–log slope)")
    axs[0, 1].axhline(0.0, color="gray", linestyle="--", linewidth=0.8)
    axs[0, 1].grid(True, alpha=0.3)

    axs[1, 1].scatter(g_arr[valid_V], np.asarray(beta_V)[valid_V], c="C1", s=36, zorder=3, label=r"$\beta_V$")
    axs[1, 1].set_title(r"Power-law exponent $\beta_V$ vs shock level $g$")
    axs[1, 1].set_xlabel(r"Post-shock signal $g$")
    axs[1, 1].set_ylabel(r"$\beta_V$ (log–log slope)")
    axs[1, 1].axhline(0.0, color="gray", linestyle="--", linewidth=0.8)
    axs[1, 1].grid(True, alpha=0.3)

    axs[0, 2].scatter(g_arr[valid_hl_U], hl_U_arr[valid_hl_U], c="C0", s=36, zorder=3, marker="s", alpha=0.85)
    axs[0, 2].set_title(r"Half-life $U$: time to midpoint of $U(0)\to U(T)$")
    axs[0, 2].set_xlabel(r"Post-shock signal $g$")
    axs[0, 2].set_ylabel("Half-life (time units)")
    axs[0, 2].grid(True, alpha=0.3)

    axs[1, 2].scatter(g_arr[valid_hl_V], hl_V_arr[valid_hl_V], c="C1", s=36, zorder=3, marker="s", alpha=0.85)
    axs[1, 2].set_title(r"Half-life $V$: time to midpoint of $V(0)\to V(T)$")
    axs[1, 2].set_xlabel(r"Post-shock signal $g$")
    axs[1, 2].set_ylabel("Half-life (time units)")
...
def default_shock_g_grid():
    ...
    pos = [0.4, 0.3, 0.2, 0.1, .6, .8, 1, 1.3, 1.6, 1.9, 2.5]
    neg = [-.1,-.3,-.5]
